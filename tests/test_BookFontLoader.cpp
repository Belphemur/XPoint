...
void testBookFontLoaderBasics() {
  ...
  // Test 4: getReaderFont() should return non-null (will be builtin fallback initially)
  book::FontChain* readerFont = loader.getReaderFont();
  assert(readerFont != nullptr);
  std::cout << "✓ getReaderFont() returns non-null pointer" << std::endl;
  
  // Test 5: Style coverage of reader font (should be 0 for empty chain)
  uint8_t styleCoverage = readerFont->styleCoverage();
  assert(styleCoverage == 0);
  std::cout << "✓ Reader font style coverage is 0" << std::endl;
...
void testFontFaceInfoStructure() {
  ...
  BookFontLoader::FontFaceInfo faceInfo;
...
void testFamilyInfoStructure() {
  ...
  BookFontLoader::FamilyInfo familyInfo;
...
void testStyleFlags() {
  ...
  assert((StyleBold & StyleItalic) == 0); // Bold and Italic shouldn't overlap
  std::cout << "✓ Bold and Italic flags don't overlap" << std::endl;
  
  // Test 2: Combined style flags
  uint8_t boldItalic = (StyleBold | StyleItalic);
...
void testConstants() {
  ...
  assert(BookFontLoader::kMaxDiscoveredFamilies > 0);
  assert(BookFontLoader::kMaxDiscoveredFamilies <= 100); // Reasonable upper bound
  std::cout << "✓ kMaxDiscoveredFamilies = " << (int)BookFontLoader::kMaxDiscoveredFamilies << std::endl;
