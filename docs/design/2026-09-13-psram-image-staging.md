...
| 2026-09-13 | PNGdec bumped to 1.1.7 (folded from PR #110) | The PSRAM memory-decode path sits on PNGdec's `openRAM` seam, so the dependency bump belongs in this PR. 1.1.7 is additive only — it adds tEXt chunk comment retrieval (`PNGIMAGE::szComment` + `PNG::getComment()`, upstream commit `64f9f3b`); no behavioral change to `openRAM`/read/seek/draw callbacks, and both env builds plus all PNG host-test units are green. PlatformIO's registry still only publishes 1.1.6, so the pin uses the upstream git commit SHA `64f9f3bc11cc1acb49f5bce823baae6ece910ba6` (immutable, matching the JPEGDEC pin pattern; a mutable tag would be a supply-chain substitution vector for OTA-signed firmware). |
