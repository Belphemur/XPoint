...
> **E.5 crash-recovery decision (2026-09-04):** No active-loop checkpoint. On power loss mid-build, the next open re-enters `loadSectionFile()` which returns false (no valid `.bin`), then `EpubReaderActivity` falls back to `Section::createSectionFile()` → `startBuild()` → `buildSomeMore(0)`. The rebuild reuses the on-SD HTML cache from Phase 2 (`html/<spineIndex>.html`, written once at chapter open), so a full rebuild is ~1-2 s — much cheaper than a first build. **An additional `partial-<N>` snapshot every 5 s is rejected** (CodeRabbit P_h5) because (a) it costs an extra ~720 SD writes per hour of reading for work that is recoverable on restart, and (b) the project's standing design goal is to *minimise SD writes* (see §0 design motivation) — extra temp files for recalculable work are a regression. The 5 s log-only "checkpoint" stub in `buildSomeMore()` is **also being removed** in `95e4aa0f` follow-up because the log message misleads the operator into thinking a checkpoint happened when nothing was written.
