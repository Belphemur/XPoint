...
| The FreeType amalgam (14 modules after SDK PR #26 added `cff` + `psaux`) already compiles in every firmware build; it is dead-stripped at link because nothing references `FtFont` | `FreeInkFont/library.json` build flags; verified 2026-09-17: no `FtFont` symbols in the x4pro `.elf` |
...
| FT stack depth overflows small MCU task stacks | The smooth rasterizer's worker — including the render pool — lives ON THE CALLER'S STACK (ftgrays.c); the 16 KB default pool made every load/rasterize burn ~16 KB. SDK PR #27 shrinks FT_RENDER_POOL_SIZE to 4096 (identical output, banded sweeps): host-measured rasterize stack 18 KB → 5.7 KB for TrueType/variable faces. For CFF, the Adobe engine interprets charstrings with an UNBOUNDED multi-KB caller stack regardless of FT_LOAD_NO_HINTING (on-device: cf2_doStems → cf2_arrstack_push blew a 16 KB render task, Cache/MMU fault). SDK PR #29 tried selecting the old "freetype" CFF engine (bounded, iterative) — reverted by SDK PR #30: the Adobe engine is measurably faster and all rendering moved to ONE task instead. Final firmware architecture: the ActivityManagerRender task is GONE; every render runs on the Arduino loop task with a 48 KB stack (EPub-InkPlate proves 40 KB handles arbitrary books on the Adobe engine; 48 KB adds ChapterLayout+paint margin) — net task-stack budget unchanged (was 32 KB loopTask + 16 KB render task). Rebuilds are UX-modal anyway ("Indexing" popup), so blocking the loop is accepted. requestUpdate(true)/requestUpdateAndWait() render synchronously from the main thread (progress callbacks inside tight OTA/SD-flash loops rely on it; the flash driver critical-sections its writes, so display work between writes is safe). Per-render stack high-water telemetry + loopTask census + heap-integrity sentinel telemetry under the FT flag catch any remaining offender by name |
...
| FT has no cross-call glyph raster cache (stb cached GlyphSlots in the arena; D6 removed it) | Per-pass re-decode + re-raster of painted glyphs is the accepted Phase-1 cost; the glyphBounds band cull (SDK PR #26) skips rasterization entirely for out-of-band glyphs on plane passes, partially offsetting it. Device soak measures wall-time/loop-duration vs the stb build; if a material regression shows up, a small per-face LRU glyph cache (keyed cp\|sizePx) is a follow-up — not speculative Phase-1 complexity |
