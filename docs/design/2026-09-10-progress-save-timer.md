...
A small `ProgressSaver` owns a FreeRTOS task:
...
- **Trigger:** a repeating `esp_timer` (interval = FLUSH_INTERVAL_MS). The
  timer ISR callback does exactly one thing: `xTaskNotifyFromISR()`. The ISR
  never touches SD, the storage mutex, or reader state.
- **Task loop:** block on `xTaskNotifyWait()` with a timeout of
  FLUSH_INTERVAL_MS (so an explicit flush-now notification skips the wait).
  On wake: if dirty, take `lastSaved` under a brief critical section, build
  the record, one `ProgressFile::writeAtomic` through `HalStorage` (its mutex
  serializes with the main task's other SD access). Update `lastFlushed` on
  success; leave dirty set on failure so the next tick retries.
- **Prompt-flush semantics:** `captureCandidate()` does not wake the task; a
  candidate captured mid-interval flushes at the next timer fire (≤60 s
  later). The "flush within one interval of any change" property holds
  regardless of when in the interval the turn happened. (An earlier draft
  claimed "~1 s later" via task notification on capture — dropped as
  unnecessary: notification on capture adds wake churn for zero user-visible
  benefit, since the interval bounds the window either way.)

**Memory:** ~2 KB task stack, one 16-byte pending record, one 16-byte
`lastFlushed` record, static (no heap after task creation at boot).
