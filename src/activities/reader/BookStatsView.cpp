...
bool shouldShowRtcBasedStats() {
  ...
  return getCurrentLocalReadingStatsDateTime(today);
}
...
void drawGlobalStatsCard(const GfxRenderer& renderer, const int x, const int y, const int w, const int h,
  ...
  if (showRtcStats) {
    ReadingStatsDateTime today;
    const bool hasToday = getCurrentLocalReadingStatsDateTime(today);
    const uint16_t currentStreak = hasToday ? stats.currentReadingStreakDays(&today.date) : 0;
    if (currentStreak > 0) {
...
void renderPerBookStatsPage(GfxRenderer& renderer, const MappedInputManager* /*mappedInput*/,
                            const std::string& bookTitle, const BookReadingStats& stats, const float progressPercent,
                            const bool hasEstimatedTimeLeft, const uint32_t estimatedTimeLeftSeconds,
...
void renderGlobalStatsPage(GfxRenderer& renderer, const MappedInputManager* /*mappedInput*/, const char* screenTitle,
  ...
  if (showRtcStats) {
    ...
    drawGlobalStatsCard(renderer, cardX, y, cardW, globalCardH, screenTitle, stats, layout);
