...
void BaseTheme::applyHeaderStatus(const GfxRenderer& renderer, freeink::ui::HeaderProps& props) {
  ...
  // Header clock (date + HH:MM), opposite the battery, on every screen that
  // draws this band (SETTINGS.headerClock). Themes whose title layout has no
  // room for the clock's left reserve opt out via headerShowsClock. Only drawn
  // when an RTC is present — without one the system clock is never seeded, so
  // any value shown would be meaningless.
  static char clockText[ClockFormat::kDateAndTimeSize];
...
void BaseTheme::drawStatusBar(GfxRenderer& renderer, const float bookProgress, const int currentPage,
  ...
  if (sb.showsClock() && halClock.isAvailable()) {
    char timeBuf[9];
    if (halClock.formatTime(timeBuf, sizeof(timeBuf), sb.clockUtcOffsetMin, sb.clock12h)) {
