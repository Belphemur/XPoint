...
| | |
|---|---|
| **Status** | Draft — reviewed against codebase 2026-09-08 |
...
Every file path, class name, and function signature cited below was verified against the code in this worktree on 2026-09-08. Line numbers refer to the current revision and may drift.
...
| `SdCardFontSystem` | `src/SdCardFontSystem.{h,cpp}` | facade: `begin(GfxRenderer&)`, `ensureLoaded(GfxRenderer&)`, `resolveFontId(family, pt)` :27, `registry()`, `markRegistryDirty()`/`refreshIfDirty()` | EXTENDED (§3.6): also fronts TTF fonts |
...
| `EpubReaderActivity` | `src/activities/reader/EpubReaderActivity.{h,cpp}` (3280 lines) | navigation, prefetch (`prefetchNextChapterDuringDisplay` :478), heap gates (`BACKGROUND_BUILD_MIN_FREE_HEAP=32KB`, `RENDER_MIN_FREE_HEAP=24KB` :147-154), toolbar/footnotes/dictionary/stats | KEPT; page source swapped (§3.5) |
...
- `EpdFontFamily::Style` (bits: BOLD=1, ITALIC=2) is **bit-identical** to FreeInkBook `StyleFlags` (`StyleBold=1, StyleItalic=2` in `include/BookFont.h`) — style mapping is a cast, no translation table.
...
Library is present as a submodule but **not yet linked**: `platformio.ini` `lib_deps` (:120-148) includes `FreeInkUI=symlink://freeink-sdk/libs/ui/FreeInkUI` (:133) but nothing from `freeink-sdk/libs/book/FreeInkBook/`. FreeInkBook is C++17, freestanding (no exceptions/RTTI), depends only on its caller-provided `Arena` buffers.
...
| `TtfFont` | `include/render/TtfFont.h`, `src/render/TtfFont.cpp` | `bool init(const uint8_t* data, uint32_t len, Arena& glyphArena)` — borrows `data`, which must outlive the font; 32–64KB glyph arena is comfortable for one active size. Direct-mapped advance/glyph tables **append** to the arena; when it fills the cache flushes entirely and rebuilds (no LRU). stb_truetype requires the **whole font file addressable in memory**. |
| `FontChain` | `include/render/TtfFont.h` | `bool add(RenderFont*, uint8_t styleFlags)` (≤8 faces); `RenderFont* fontFor(cp, styleFlags, uint8_t* faceFlagsOut)` — selection: exact style with glyph → partial → regular → anything with glyph; `styleCoverage()` bitmask (for fingerprints); metrics from the first registered face; kerning only when both codepoints resolve to the same face. **No synthetic bold/italic** — `faceFlagsOut` lets a renderer know the chosen face is not the requested style. |
...
| `PageRenderer` | `include/render/PageRenderer.h`, `src/render/PageRenderer.cpp` | `FrameTarget{framebuffer, width, height, widthBytes, format, rotation}`; `FrameFormat::Mono1Dithered` (1bpp MSB-first, SET=white; glyphs carry real 8-bit coverage consumed as e-paper-tuned dithered edge AA — engine default), `Mono1Sharp` (coverage ≥120 hard threshold, no edge softening), `Gray8` (8bpp `lum = 255-coverage`; unused here, §1); `renderText(page, fontChain, target, &firstMissing)`, `renderImages(...)` (Bayer dither), `render(page, fontChain, source, zip, scratch, target)` |
...
| `BookProfile` | `include/BookProfile.h` | `-DFREEINK_BOOK_SMALL=1` (PSRAM-less, ~200KB-free-heap class) or `-DFREEINK_BOOK_LARGE=1`, mutually exclusive; **there is no `-DFREEINK_BOOK_PROFILE` flag** — capacities live at use sites (`ChapterLayout.cpp`, `TtfFont.h`) |
| `BitmapBookFont` / `TtfGlyphSource` | FreeInkUI `include/FreeInkUIBookFont.h` | `BitmapBookFont(const BitmapFont& = kNotoSansFont) : book::RenderFont` — adapter for the existing bitmap fonts (metrics ignore sizePx; ~4KB `coverage_` per instance → must live in static storage); `TtfGlyphSource` registered via `DisplayTarget::setGlyphFallback` for UI text |
...
| Style | `EpdFontFamily::Style` (0..3) | `StyleFlags` (bits 0-1 identical) | plain cast |
...
| Page model | `Page` of `PageLine{TextBlock}` with selection groups, focus split, ruby, link spans | `Page{PageTextRun[], PageImage[], PageLink[]}` + `charStart` anchors | **feature parity gaps**: selection groups, ruby, focus-reading word splits are not in FIBP text runs (§8, §11) |
...
Heap pressure data points: `~380KB` usable DRAM on C3 (project constraint), the wolfSSL memory comment cites *"the ~50KB free heap a reading session leaves"* (platformio.ini:60-62), and `EpubReaderActivity` gates builds at 32KB free / 16KB max-alloc (:147-150). The `firmware_tuned` core rebuild reclaims ~32–37KB (:158-183).
...
Built-in fallback: a static `book::FontChain` wrapping static `BitmapBookFont` instances over the built-in bitmap font data (one instance per *style* — the type's metrics ignore `sizePx`; its 4KB `coverage_` buffers must live in static storage, per `FreeInkUIBookFont.h`). When `SETTINGS` selects a built-in family, or a TTF load fails, `getReaderFont()` returns this chain. This keeps the reader functional on day one with **zero** TTFs on the card, and gives the release builds a safety net.
...
C3 steady state during a build tick ≈ 32 + 40 + 24 + 8 = **104KB of caller arenas**, plus the borrowed framebuffer. This is only feasible because:
...
**Font size gate (C3):** before `TtfFont::init`, require `fileSize ≤ kMaxDramFontBytes (256KB) - safety`. A 700KB Latin serif will not fit next to the framebuffer, the stack, and WiFi leftovers on a C3; the loader logs `LOG_ERR("BFNT", "Font %s too large for DRAM tier (%u > %u)", ...)` and keeps the family listed but greyed out in the Font tab (§12) rather than pretending to support it. On the PSRAM tier the gate is the free-PSRAM probe instead. This gate is also what keeps stb_truetype's whole-file-addressable requirement honest until a streaming (FreeType) backend arrives behind the same `BookFont` interface (`freeink-sdk/docs/freeink-book.md`).

**Glyph cache behavior note:** when the glyph arena fills, `TtfFont` flushes the entire cache and rebuilds (no LRU). At C3 sizes (24KB) on a book mixing scripts this can thrash; the mitigation is the same as today's `.cpfont` prewarm economics — one active size, per-page codepoint sets, and FIBP caching meaning a cached page never re-rasterizes. Measured via `Arena::highWater()` / `failedAllocSize()` logging in Phase 2 (§9).
...
| `focusReadingEnabled` | `focusReading` | engine-side bold-prefix emphasis |
...
**Feature-parity note (must be tracked, not hand-waved):** FIBP `PageTextRun` carries text+position+style only. The current TextBlock page model also carries selection groups (dictionary word hit-testing), per-word focus splits (engine has paragraph-level `focusReading` instead), ruby texts, and link spans. Mapping: footnotes/links ride `PageLink` + `onAnchor`; dictionary hit-testing recomputes from run text + run x positions (same info `DictionaryWordSelectActivity` uses, available from the cached runs); ruby and per-word focus splits are deferred (§8 R6, §11 Q3). This is the honest cost of the migration and it is scoped, not silent.
...
Style availability is honest: a family whose chain registers only `Regular` still works — `FontChain::fontFor(cp, StyleBold, &faceFlagsOut)` falls back to the regular face and reports the shortfall via `faceFlagsOut`; the fingerprint includes `styleCoverage()` so installing the real bold file later triggers correct re-layout (§3.4). Whether `PageRenderer::renderText` stroke-simulates the missing style from `faceFlagsOut` must be verified in Phase 2; if it does not, bold rendering degrades to regular glyphs and the doc's risk table carries it (§8 R7).
...
| 8 | **Style bits map 1:1.** | `EpdFontFamily::Style` (EpdFontFamily.h:10-14) and `BookFont::StyleFlags` share bit layout (BOLD=1, ITALIC=2); conversion is a cast with a static_assert. |
| 9 | **`textAntiAliasing` maps to `FrameFormat::Mono1Dithered` (ON) / `Mono1Sharp` (OFF); dual-plane 4-level gray has a no-engine-change fix path via tiled Gray8 re-render.** | The engine rasterizes true 8-bit glyph coverage (`stbtt_MakeGlyphBitmap`, TtfFont.cpp:165) and consumes it per `FrameFormat` (PageRenderer.cpp:67-95): dithered-contrast 1bpp (default; `kInkSolid=140`/`kInkFloor=40` contrast curve + Bayer, "stems stay black, edges stay smooth"), hard-threshold 1bpp (`kInkThreshold=120`), or Gray8. `SETTINGS.textAntiAliasing` (CrossPointSettings.h:289, default ON) keeps its existing Style-row toggle (TextSettingsActivity.h:33) with real hardware meaning. The bitmap reader's 4-level gray — 2-bit glyph tones through `lib/GfxRenderer/GrayPlanes.h` into dual LSB/MSB plane RAM + the panel AA waveform (`displayGray()`, e.g. the UC8279 XTF_AA LUT bank) — has no engine `FrameFormat` equivalent. **Fix path (no engine changes, §11 Q7)**: `PagePaint` renders the grayscale pass by calling `PageRenderer::renderText` with `FrameFormat::Gray8` into a tiled strip buffer (reusing `GfxRenderer::beginStripTarget`, 80-row bands), then quantizes the 8-bit coverage to 2-bit tones (≥192→3, ≥128→2, ≥64→1) and runs through the existing `grayplanes::planBlock` + `drawGrayDualPixel` pipeline — same tone range `grayplanes` already expects from `EpdFont` is2Bit glyphs, tiling avoids the 384KB full-page Gray8 buffer problem on C3. Trade-off: double rasterization of TTF glyphs per grayscale pass (same cost the bitmap path already pays). Gray8 as a page-wide format stays out of scope. |
...
| `lib/Epub/Epub/Section.{h,cpp}`, reader `Page`/`PageLine` model (`lib/Epub/Epub/Page.{h,cpp}`) | replaced by FIBP; **only after** `TxtReaderActivity` + dictionary flows are off them |
...
| Torn cache file (power loss mid-write) | `CacheStorage` temp+rename on `endWrite()`; `readAt` short reads | partial file accepted by design (`isPartial()`); unreadable file → `remove()` + rebuild |
...
|---|---|---|---|---|
| R1 | **C3 PSRAM-less budget insufficient** (arenas + fonts + WiFi leftovers) | Medium | High | Two-tier design (§3.3); SMALL profile; framebuffer-loaned `parseScratch`; heap gates carried over; per-page FIBP so cached pages never re-render. Measured gates in Phase 2; descope slider = smaller `glyphArena` first, then `scratch`. |
| R2 | **CJK/very large TTF cannot meet whole-file residency** on C3 | High (eventually) | Medium | Hard size gate + UI honesty (§3.3); CJK readable via bitmap fallback chain (retained `SdCardFontSystem` CJK path); FreeType streaming upgrade documented behind `BookFont`. |
| R3 | **Feature parity gaps** (ruby, per-word focus splits, selection groups) | High | Medium | Scoped in §3.5 note; paragraph-level `focusReading` covers focus reading; ruby deferred with an issue; dictionary hit-testing recomputed from runs. No silent loss. |
| R4 | **Glyph-arena thrash** (flush-and-rebuild, no LRU) on mixed-script pages at C3 sizes | Medium | Medium | FIBP pages amortize; monitor `Arena::highWater()/failedAllocSize()` in Phase 2 logs; raise `glyphArena` budget if logs show rebuild storms. |
...
| R7 | **Synthetic style rendering**: missing bold/italic faces degrade | Medium | Low | `styleCoverage()` in fingerprint keeps caches correct; UI labels families honestly (§12); upstream synthetic-stroke decision tracked (§5.5b). |
| R8 | **Engine unproven on C3 single-core RISC-V @160MHz** | Medium | High | Phase 1 debug activity + Phase 2 flag-gated soak; per-page timing logs alongside the existing `BOOK_PROFILE` phase logs (EpubReaderActivity pattern, :479-519). |
| R9 | **AA parity regression vs the bitmap path**: TTF pages render 1bpp dithered edges; the bitmap path draws true 4-level gray (dual-plane) when `textAntiAliasing` is on | Certain (v1) | Low | `Mono1Dithered` is the engine's e-paper-tuned default (contrast curve keeps stems solid, edges dithered, PageRenderer.cpp:22-23); accept for v1. The §11 Q7 fix path (tiled Gray8 re-render → 2-bit quantization → grayplanes) eliminates this gap with no engine changes; Phase 2 soak confirms the double-rasterization cost is acceptable on C3. OFF → `Mono1Sharp` keeps the settings row honest for users who prefer hard edges. |
...
5. *Power-loss*: cut power during a cold build; reopen → partial FIBP accepted, build resumes, no torn-file crash (exercises `CacheStorage` temp+rename).
...
3. **Ruby + per-word focus splits**: acceptable to defer to a follow-up (paragraph-level `focusReading` interim), or a blocker for any user cohort?
...
7. **AA format for TTF pages**: ship with `Mono1Dithered`/`Mono1Sharp` (§5 D9) and file the upstream dual-plane `FrameFormat` ask (matching `lib/GfxRenderer/GrayPlanes.h` LSB/MSB plane bits), or accept dithered 1bpp AA as final? Does the panel's 4-level AA waveform (UC8279 XTF_AA LUT bank, SSD1677 dual DTM planes) visibly beat dithered coverage at 12–18 pt reader sizes on X4/X3 panels?

**Concrete fix path (no engine change required):** the gap is that `PageRenderer::renderText` writes 8-bit coverage to a `FrameTarget` via `inkPixel` (PageRenderer.cpp:165-209), which only handles `Mono1Dithered`/`Mono1Sharp`/`Gray8` — there is no dual-plane output path. CrossPoint's 4-level gray lives in `GfxRenderer`'s `grayplanes` system (`GrayPlanes.h`, `renderCharImpl` GRAYSCALE_DUAL at GfxRenderer.cpp:540-545), driven by 2-bit glyph tones from `EpdFont`, not by TTF coverage. The fix: `PagePaint` renders the grayscale pass by calling `PageRenderer::renderText` with `FrameFormat::Gray8` into a **tiled strip buffer** (reusing `GfxRenderer::beginStripTarget`'s band mechanism, 80-row strips at EpubReaderActivity.cpp:1982), then quantizes each 8-bit coverage value to a 2-bit tone (≥192→3, ≥128→2, ≥64→1, else 0) and runs it through the existing `grayplanes::planBlock` + `drawGrayDualPixel` pipeline. This avoids both the 384KB full-page Gray8 buffer problem on C3 (tiling keeps it to one band at a time) and any engine-side FrameFormat addition. The quantized tone values are the same 0-3 range `grayplanes::planBlock` already expects from `EpdFont` is2Bit glyphs. Trade-off: re-renders TTF glyphs for the gray pass (double rasterization cost per page), same as the current bitmap path already does. Phase 2 soak measures whether the extra engine work is acceptable on C3.
...
**Font tab (Tab::Family)** — today: `Noto Serif`, `Atkinson Hyperlegible Next` (`VISIBLE_BUILTIN_FONT_COUNT = 2`, TextSettingsActivity.cpp:37/81), then a `SD Card Fonts` section listing registry families. Proposed: keep all of it, append a `TTF Fonts` section listing `BookFontLoader::families()`:
...
**Style tab (Tab::Style)** — today: Focus Reading, Hyphenation, Embedded styles, Anti-aliasing (StyleRow, TextSettingsActivity.h:33; the row toggles `SETTINGS.textAntiAliasing`, CrossPointSettings.h:289, default ON). Proposed: the Anti-aliasing row **stays visible on TTF families** — it is real hardware capability, not a bitmap-only nicety. ON → `FrameFormat::Mono1Dithered` (engine-native dithered edge AA from 8-bit glyph coverage, zero extra RAM), OFF → `Mono1Sharp` (hard coverage threshold, crispest stems) (§5 D9). Honest parity note: the bitmap path renders 4-level gray edges via dual-plane panel waveforms (`GrayPlanes.h` → `displayGray()`); TTF pages use dithered 1bpp until the engine grows a dual-plane format (§8 R9, §11 Q7). Focus Reading maps to engine `focusReading` (paragraph-level). Family style coverage ("Regular · Bold · Italic" availability) surfaces in the Font tab, not here.
...
- Engine design doc: `freeink-sdk/docs/freeink-book.md` (authoritative; note: `TtfFont.h`'s comment references a stale `docs/freeink-book-design.md` filename)
- Bitmap font tooling (the constraint TTF replaces): `lib/EpdFont/scripts/fontconvert_sdcard.py`, `lib/EpdFont/scripts/cpfont_version.py`, `lib/EpdFont/scripts/build-font-ids.sh`, and the Hermes skill reference `references/font-regen-gotchas.md` (skill `software-development/crosspoint-reader-dev`; the bitmap regeneration reproducibility gate this design supersedes)
- Research: `/home/balor/.hermes/webui/attachments/1d8cb3219c1a/TTF_Font_Rendering_on_ESP32-C3.md` (LVGL/TinyTTF, FreeType, stb, PSRAM `MALLOC_CAP_SPIRAM` strategies)
