...
| R9 | **AA parity regression vs the bitmap path**: TTF pages render 1bpp dithered edges; the bitmap path draws true 4-level gray (dual-plane) when `textAntiAliasing` is on | Certain (v1) | Low | `Mono1Dithered` is the engine's e-paper-tuned default (contrast curve keeps stems solid, edges dithered, PageRenderer.cpp:22-23); accept for v1, file the dual-plane `FrameFormat` upstream ask (§5 D9), decide in §11 Q7 after side-by-side comparison in the Phase 2 soak. OFF → `Mono1Sharp` keeps the settings row honest for users who prefer hard edges. |
